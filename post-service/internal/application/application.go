...
import (
	...
	"github.com/BernardN38/social-stream-backend/post-service/internal/router"
...
func NewApp() *App {
	...
	//listen for messages
	mr := messaging.New(rabbitmqChannel)

	//start service layer
	service := service.NewService(db, mr)

	//create request handler

	hanlder := handler.NewHandler(service)

	tokenAuth := jwtauth.New("HS512", []byte(config.JwtSecret), nil)
	//create request router
	router := router.NewRouter(hanlder, tokenAuth)
	return &App{
		Router: router,
	}
}
